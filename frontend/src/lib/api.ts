...
export const transactionsApi = {
  async getTrades(skip: number = 0, limit: number = 100, accountId?: number): Promise<Trade[]> {
    ...
    const response = await fetch(`${API_BASE_URL}/api/v1/transactions/trades?${params.toString()}`);
    if (!response.ok) {
  ...
  async getCashJournal(skip: number = 0, limit: number = 100, accountId?: number, type?: string): Promise<CashJournal[]> {
    ...
    const response = await fetch(`${API_BASE_URL}/api/v1/transactions/cash-journal?${params.toString()}`);
    if (!response.ok) {
  ...
  async getFxTransactions(skip: number = 0, limit: number = 100, accountId?: number): Promise<FxTransaction[]> {
    ...
    const response = await fetch(`${API_BASE_URL}/api/v1/transactions/fx-transactions?${params.toString()}`);
    if (!response.ok) {
  ...
  async getCorporateActions(skip: number = 0, limit: number = 100, accountId?: number): Promise<CorporateAction[]> {
    ...
    const response = await fetch(`${API_BASE_URL}/api/v1/transactions/corporate-actions?${params.toString()}`);
    if (!response.ok) {
...
export const portfoliosApi = {
  ...
  async getPortfoliosSimple(options?: { skip?: number; limit?: number; active_only?: boolean }): Promise<PortfolioSimple[]> {
    ...
    const query = params.toString();
    const response = await fetch(`${API_BASE_URL}/api/v1/portfolios/simple${query ? `?${query}` : ''}`);
    if (!response.ok) {
